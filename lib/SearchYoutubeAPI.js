...
function CheckIfVideo(component){
    if(Object.keys(component)[0]=='videoRenderer'){
        return component;
    }
}
...
function SearchKeyword(keyword) {
    return new Promise((resolve, reject) => {
        request({
            ...
            if(err)reject(err);
            var result = JSON.parse(body).contents.twoColumnSearchResultsRenderer.primaryContents.sectionListRenderer.contents[0].itemSectionRenderer.contents.filter(CheckIfVideo);;
            var videos = [];
            for(var i = 0; i < result.length; i++){
                videos.push({
                    ...
                    duration: parseTimeFromString(result[i].videoRenderer.lengthText.simpleText)
                })
            }
            resolve(videos)
        })
