...
class Guild {
    ...
    // use this only when audioplayer emit Idle
    async skip(){
            ...
            try {
                switch (interaction.customId) {
                    ...
                    case "skipButton":
                        ...
                        await interaction.update({ content: res ? res : `skipped!`, embeds: [embedmsg()], components: res?[row]:[] })
                        break;
                    case "volumeUp":
        ...
        if (playlistPattern.test(input)) {
            ...
            if (videoPattern.test(input)) {
                await ytdl.getInfo(input).catch(err => {
                    ...
                    song = new Song(result.player_response.videoDetails.title, result.player_response.videoDetails.lengthSeconds, input, channel, username, result.player_response.videoDetails.thumbnail.thumbnails[result.player_response.videoDetails.thumbnail.thumbnails.length-1].url)
                    guilds[channel.guildId].songs.push(song);
                    return song;
                });
            } else {
